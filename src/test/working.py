...
def get_ml_recommendation(available_df, drafted_players, my_team):
    """Llama a la API de ML para obtener la mejor recomendación"""
    try:
        # Ya no necesitas limpiar aquí si lo hiciste globalmente
        players_data = available_df.to_dict('records')
        my_team_positions = [p['Position'] for p in my_team]
        
        payload = {
            "players": players_data,
            "drafted_players": list(drafted_players),
            "my_team_positions": my_team_positions
        }
...
def load_data():
    ...
    # 🔧 LIMPIEZA COMPLETA DENTRO DEL CACHE
    ...
    # Rellenar NaN con string vacío en columnas de texto
    text_cols = df.select_dtypes(include=['object']).columns
    df[text_cols] = df[text_cols].fillna('')
    
    # Ordenar por FPTS
    df = df.sort_values('FPTS', ascending=False).reset_index(drop=True)
...
with tab1:
    ...
    with col4:
        use_ml = st.checkbox("🤖 ML Model", value=True, help="Usa predicciones ML")
    ...
    if st.button("Start / Reset Draft"):
        ...
        st.session_state.draft_state['roster_size'] = 13
        st.rerun()

    if st.session_state.draft_state['started'] and not st.session_state.draft_state['finished']:
        
        available_df = df[~df['Player'].isin(st.session_state.draft_state['drafted_players'])].copy()
        ...
        if is_user:
                ...
                if use_ml:
                    with st.spinner("🤖 Analizando con Machine Learning..."):
                        ml_recommendation = get_ml_recommendation(
                            available_df,
                            st.session_state.draft_state['drafted_players'],
                            st.session_state.draft_state['my_team']
                        )
                    
                    if ml_recommendation:
                        st.success(
                            f"🤖 **ML Recommendation:** **{ml_recommendation['best_player']}** "
                            f"({ml_recommendation['position']}) - "
                            f"Predicted: {ml_recommendation['predicted_fpts']:.1f} FPTS"
                        )
                        
                        if ml_recommendation.get('alternative_picks'):
                            with st.expander("🔍 Ver más opciones"):
                                for i, alt in enumerate(ml_recommendation['alternative_picks'], 1):
                                    st.write(f"{i}. **{alt['player']}** ({alt['position']}) - {alt['predicted_fpts']:.1f} pts")
                    else:
                        best_available = available_df.iloc[0]
                        st.info(f"💡 Top Available: **{best_available['Player']}** ({best_available['Position']}) - {best_available['FPTS']} pts")
                else:
                    best_available = available_df.iloc[0]
                    st.success(f"💡 Recommended: **{best_available['Player']}** ({best_available['Position']}) - {best_available['FPTS']} pts")
            ...
            for i, tab in enumerate(pos_tabs):
                with tab:
                    pos = positions[i]
                    tab_df = available_df if pos == "ALL" else available_df[available_df['Position'] == pos]
                    
                    if tab_df.empty:
                        st.warning(f"No {pos} available.")
                    else:
                        options = tab_df.apply(
                            lambda x: f"{x['Player']} | {x['Position']} | {x.get('FPTS', 'N/A')} pts", 
                            axis=1
                        )
                        selection = st.selectbox(f"Select {pos}", options, key=f"sel_{pos}")
                        
                        if st.button(f"✅ Draft ({pos})", key=f"btn_{pos}", type="primary"):
            ...
            if player_drafted:
                ...
                st.session_state.draft_state['draft_log'].append({
                    ...
                    'fpts': player_data.get('FPTS', 0),
                    'fpts_g': player_data.get('FPTS/G', 0)
                })
    ...
    if st.session_state.draft_state['draft_log']:
        ...
        st.dataframe(
            display_df.sort_values('Pick', ascending=False).head(10), 
            hide_index=True, 
